...
- [The Conversation List (Sidebar)](#the-conversation-list-sidebar)
...
- [Coming Up: Another Claude, Querying Yours](#coming-up-another-claude-querying-yours)
...
Install done, first fetch streaming. Open the browser, and the sidebar is the first thing your eye lands on; that's where the tour starts.
...
The checkbox detects compactions across every source the corpus carries: Claude Code's native `isCompactSummary` flag for current-shape sessions, a text-prefix fallback for legacy CC sessions that pre-date that flag, and a parallel text-prefix detector for Cowork (which never carried `isCompactSummary` at all). The same checkbox also filters the title-leak case, where a continued session inherits a *"Continuation of …"* title derived from a prior compaction. One checkbox, every source.
...
A second focus rule applies inside the detail pane, finer-grained than the pane-level one above. When you click a message bubble or scroll the conversation manually, you've **demonstrated focus** on whatever you're reading; from that moment on, background state changes (a header checkbox flipping, a refresh that rebuilds the corpus, an auto-promote of match zero after a fresh search) must not yank the viewer off your click. Only explicit navigation gestures move the viewer: `⌘+G` and `⌘+Shift+G`, `Enter` on a highlighted search hit, clicking a search result card, and clicking a bookmark. Everything else preserves where you are.

This rule sounds small until you've used a UI that didn't have it. The user-facing payoff is "the app respects what I just did": a search refetch that completes in the background doesn't scroll you off the bubble you're reading, and toggling Show Tools mid-skim doesn't lose your place.

The mechanism behind that guarantee fits in a paragraph, and every piece of it earns its keep against a different way the viewer would otherwise lose your place. Worth understanding because the same shape (a piece of "where the user is right now" state plus a gated effect plus explicit re-arm signals) generalizes to every UI that has to coordinate background refetches with where the user is reading.

The implementation tracks one piece of state: the UUID of the message you last clicked or scrolled to. The viewer keeps it in a React ref that the rest of the component reads on every render, so a search refetch or a checkbox flip can't accidentally lose your place; refs survive re-renders without subscribing to them. The auto-promote effect that would otherwise scroll you to match 0 of a fresh search has four gates that all have to hold before it fires: the search has to have finished (so it doesn't jump mid-keystroke), there have to be results to promote, the user can't have already navigated by `⌘+G` to a specific match (a refetch shouldn't yank you off the match you stepped to), and the demonstrated-focus ref has to be null (the gate that catches the click-then-refetch race). Three explicit user actions clear the ref and re-arm the auto-promote: typing a new query, pressing `⌘+G`, and clicking a search result card. Part 6 walks through the Council debate (ref-only versus ref-plus-state-mirror) that produced this design.
...
Both search surfaces (the title-search input at the top of the left sidebar and the right-pane full-text search) honor whatever scope the sidebar is currently showing. That includes the **source dropdown** (`All Conversations` | `Claude Desktop` | `Claude Code` | `Claude Cowork`), the **workspace dropdown** (for Claude Code sessions, scoping you to a single project), and the **active filter** (any of your saved filters from the *Manage Filters* modal). Search results also respect the **Show Tools** checkbox in the conversation header, so a hit you couldn't see in the viewer never shows up in the result list either. The **Show Compactions** checkbox composes the same way: turn it off and a snippet that lives inside a `/compact` summary disappears from the result list along with the summary card itself. Every active filter narrows the result set further.
...
| Warm conversation switch, 4,000-bubble CC session (render wall time) | ~10,300 ms | **≈514 ms** | ~20× |
...
#### Tool-aware projection via a second indexed column

Search hits sometimes lit up text the user couldn't see. You clicked a sidebar result, the conversation pane scrolled to the owning message, and the highlighted word was nowhere on screen — the match lived inside a hidden tool block.

The first FTS5 cut shipped this as a known divergence between the fast path and the linear-scan fallback under the **Tools** toggle. The viewer hides tool_use and tool_result bubbles by default; the search index always stored the full projection (tool blocks included), so `snippet()` occasionally highlighted a token that lived only inside a hidden tool block. The linear-scan fallback got this right because it re-projected the body at query time and filtered tool blocks before placing the highlight.
...
#### Truncation disclosure on every response
...
#### Cold-restart lifespan staggering
...
#### Event-driven freshness while running
...
The schema bumps continue past v9 (the `body_text` projection and dedupe fixes covered above) into v10–v14; the second wave of wins (bubble-list virtualization for the warm conversation switch, gzip-middleware bypass on the conversation-detail route, lifespan-startup warmups, and a self-healing migration that recovered Cowork rows orphaned by an earlier bump) plus the search-typing-storm postmortem they came out of all live in Part 7.

## Inside the Conversation Pane

With search and the keyboard fast and the sidebar painting at first paint, the rest of the work happens inside the detail pane: timestamps, image attachments, the lightbox, the local image cache, and the copy / branches / scroll-to-match affordances.
...
The export honors the same `showToolCalls` toggle as the viewer. One truth, three surfaces (viewer, copy, export); if you've decided tool calls should be visible for this session, that decision applies consistently whether you're reading in the UI, copying to your clipboard, or exporting to a file. The same goes for **Show Compactions**: the dialog used to carry its own per-export `Include /compact content` checkbox, which has since been removed in favor of inheriting the live viewer preference. One header checkbox now drives viewer visibility, Copy-as-Markdown, and both Markdown export modes.

Bundles that contain a `/compact` boundary also pick up a rich-rendering fix that landed alongside the unified checkbox. The export used to emit a stub trigger row (the literal `/compact` invocation) and drop the summary body; the current path emits the full summary the user sees in the viewer, with the heading and embedded content intact. The exported bundle now captures what the user was reading.
