...
The important design choice here is that export honors the same `showToolCalls` toggle as the viewer. One truth, three surfaces (viewer, copy, export); if you've decided tool calls should be visible for this session, that decision applies consistently whether you're reading in the UI, copying to your clipboard, or exporting to a file. Image attachments come through the same way: each one becomes a standard Markdown image reference (`![Image attachment: foo.png](/api/.../preview)`) appended to the message body. The URLs are relative to the local Claude Explorer backend, so the images render in any Markdown viewer that's loading the file *while the local server is running*; if you're going to paste the export into a serverless context (a Medium draft, a GitHub Gist, a static notes app), expect the image references to read as a list of filenames rather than to render inline. PDF is the right export when you want the image bytes embedded.
