...
Performance-wise, the tuned path is fast enough that you stop thinking about it. On a warm cache, search returns in around `50 ms`, and the conversation listing path comes back in around `0.07 s` after the performance pass (the backend uses `orjson` for parsing, an mtime-keyed `FileCache`, and parallel reads via a `ThreadPoolExecutor`). That earns one sentence because it affects how the UI feels, but it doesn't need a war story; the only thing that matters to you is that `⌘+K` feels interactive, not like you're waiting on a disk crawl.
...
Messages can contain multiple content blocks. In practice, you'll see at least four block types:
...
There's also a *"View branches"* button on the conversation header. Claude can create branches when you edit an earlier message and regenerate from there; when branches exist, the UI renders a tree visualization so you can see the structure. The visualization is intentionally read-only; it's there to help you understand the shape of the conversation, not to turn this app into a full conversation editor with branch switching and merge semantics. I love it when I get to use the word isomorphic, but I do *not* love it when I get to build a branch switcher for a corpus that almost never branches; the tree view gives you the insight without dragging the app into a whole new problem domain.
