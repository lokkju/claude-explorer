...
Beyond the source dropdown, the sidebar carries a small *named-filter* picker that lets you keep title-pattern filters around and switch between them. Each saved filter is a name plus a pattern (or a group that ANDs / ORs other named filters together — handy when you want one filter that hides chores AND keeps client-A work, instead of toggling two separate ones), and exactly one filter is active at a time. Pick *Hide work-day chores* and the list narrows; pick *All conversations* and you're back to the full corpus. The composition story matters more than it sounds: when you find yourself building the same on-the-fly filter combos repeatedly, a saved group is the cheapest way to stop re-typing them — and the active-filter selection is sticky across reloads, so the *"view"* of your archive you opened the app with is the one you come back to tomorrow.
