...
Search defaults to global, which is the behavior most people expect; you opened the app to find something across an archive, not just inside the conversation you happen to be reading. But there's a complementary mode that matters whenever you've drilled into a specific session: *"search this conversation only"* (or *"this project only,"* for Claude Code sessions grouped under a `cwd`). In Claude Explorer, that's a **pin**. There's a small `Pin search` button next to the conversation title with a dropdown — `Pin this conversation` and (when applicable) `Pin this project`. Click one and you're scoped; the SearchPanel sprouts a chip that says `In: <Conversation Title>` (or the project name), and the sidebar dims any rows that fall outside the scope so you can see at a glance what's currently in play.
