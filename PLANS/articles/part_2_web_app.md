...
Settings persist server-side rather than in browser localStorage. When you change a setting, the frontend `PATCH`es `/api/preferences`, and the backend writes the merged blob to `~/.claude-exporter/preferences.json` (atomic tmp-and-rename, `0600` permissions, deep-merge per key). The practical consequence is that your settings follow you across browsers and Incognito windows on the same machine — pick `Dark` mode and Vim navigation in Chrome, then open the same `localhost:5173` in Safari, and you get the same configuration without re-clicking anything. The frontend keeps a localStorage mirror as a fallback so the UI keeps working if the backend is briefly down (you'll lose the in-flight write, not the prior state).
