...
| Image syntax in the article series | **Standard Markdown** `![alt](Attachments/<file>.png)`, NOT Obsidian `![[...]]` embeds. GitHub does not render `![[...]]` (it shows as literal text), so the embeds broke on the GitHub surface, which is co-equal with Medium. Standard MD renders on **GitHub and Obsidian** and re-imports to Medium. Rules: (1) relative path from the article file (`Attachments/<file>`; the images live in `articles/Attachments/`); (2) **percent-encode spaces** as `%20` (`Pasted%20image%20....png`) so GitHub's link parser resolves them; (3) the image file MUST be git-committed or GitHub serves a 404; (4) drop Obsidian `|width` hints, since GitHub and Medium size images to the content column regardless (width only ever mattered in Obsidian). Standard MD has no width control, so a **narrow crop** (sidebar, modal, settings, theme toggle) must use HTML `<img src="Attachments/...%20....png" alt="..." width="N">` with the SAME relative+`%20` src; the full-window shots stay standard MD (they are meant to be column-width). The HTML-`<img>` relative+`%20` src is **confirmed rendering AND sizing correctly in Obsidian (2026-06-01 click-test)**, so it is the standard for sized crops, not a last resort. **Converted 2026-06-01** across the Part 2 twins + quickstart hero: 38 full-window/hero images as standard MD, 9 sized crops as `<img width>` (widths 250/306/400/400/450/514, restored from the original `|width` hints), and the 2 pre-existing bare-filename `<img>` tags repointed to the `Attachments/%20` src. Standard-MD and `<img>` forms both confirmed in Obsidian; GitHub render pending a push-test. **Part 3's 5 embeds were left as `![[...]]`** because their image files are not yet in `articles/Attachments/` (untracked, repo-absent); convert them once the files are added. **Publish step: at Medium import, re-insert images** (Medium re-hosts and ignores both `![[...]]` and relative paths). |
