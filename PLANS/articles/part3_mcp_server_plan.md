...
- **Honesty caveat (critical):** the series dogfooding was **front-loaded, not end-to-end**. The server mined the build session into citable `PROCESS/` artifacts **once**; later drafting + Part 2 revisions were direct file editing against those artifacts, not re-querying the store. Only **two** sessions ever touched the series (`a70251a5`, `76fe578b`).
...
4. **The Workflow That Mined This Series** — the headline. Real April-snapshot numbers; outline → 21 phases → 20 bounded pulls → synthesis → drafts. **Outline-first is the load-bearing pattern.** **Hard recursion cap:** the server is an excavator, run **once** to stabilize the build session into `PROCESS/` artifacts; drafting afterward was ordinary direct editing.
