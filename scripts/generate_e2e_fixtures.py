...
into ``tests/fixtures/`` that the Playwright e2e suite can run against,
so external contributors don't need Raymond's actual ``~/.claude-exporter/``
or ``~/.claude/`` directories to test the app.

What it produces (idempotent — re-run anytime to refresh):

  tests/fixtures/desktop/by-org/<ORG_UUID>/<UUID>.json
  ...
  tests/fixtures/claude/projects/-fixture-project/<SESSION_UUID>.jsonl
